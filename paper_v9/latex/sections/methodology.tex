...
mandated. Retrieval-Augmented Generation (RAG), in which a model conditions
its output on retrieved project or domain documents, was presented as one
domain-dependent example~\cite{lewis2020retrieval}; none of the 14 projects
implemented a full RAG pipeline. The analyzed systems spanned healthcare,
finance, education, research, ecotourism, onboarding, and tutoring domains,
and used project-specific integrations such as summarization, document/profile
...
documentation, and source code in the repository, but Git captures only
repository-visible evidence--artifacts and changes committed to version
control--rather than AI-use telemetry such as prompt logs, API calls, or IDE
acceptance events. Second, independent evaluator scorecards were collected at
$T_1$--$T_3$ through blind scoring after status presentations, with 4
evaluators in 2025.2 and 3 in 2026.1. Third,
...
in a small, mixed-scale panel; p-values are descriptive diagnostics rather than
confirmatory tests. Leave-one-out analyses recompute associations after
removing each eligible team-semester and show sensitivity to influential cases
