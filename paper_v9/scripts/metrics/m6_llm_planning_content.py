...
def generate(force: bool = False, verbose: bool = False) -> dict[str, Any]:
    ...
    metadata = {"status": "success", "gate_status": "pending_human_review", "contract_version": CONTRACT_VERSION, "metric_definition_version": CONTRACT_VERSION, "input_sha256": input_hash, "config_sha256": config_hash, "rq": "RQ3", "unit_of_analysis": "team_semester", "model": MODEL, "temperature": TEMPERATURE, "llm_calls_required": True, "coverage": {"team_semester_n": len(records), "success_n": sum(r["status"] == "success" for r in records), "unavailable_not_measured_n": sum(r["status"] == "unavailable_not_measured" for r in records)}, "limitations": ["LLM extraction is exploratory and requires human review", "literal quote validation does not establish semantic correctness", "M6b must not feed M9 before review approval"], "artifacts": [output_path.name, metadata_path.name]}
