...
All metrics derive from artifacts already produced by our data-processing
pipeline. No new data are collected and no new LLM calls are issued. For each
metric, we identify a reproducible processing path from the raw artifact to
the reported value.
...
missingness, reusing the same distribution-summary primitive Phase 2 uses for
its own ordinal scores.
...
\centering
\begin{tabularx}{\textwidth}{@{}p{0.7cm}p{4.4cm}p{2.6cm}X@{}}
\toprule
\textbf{ID} & \textbf{Metric} & \textbf{Grain} & \textbf{Source Artifact} \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ1 -- GenAI adoption and role-perception}} \\
\midrule
M1 & AI-Authorship Density (behavioral) & Team-semester $\times$ cut & \texttt{integration\_friction\_metrics.parquet} \\
M2 & AI-Dependency Trajectory (perceptual) & Cohort $\times$ cut & \texttt{textual\_cut\_signals.parquet} \\
M3 & Perceived Role-Disruption Risk & Cohort $\times$ cut & \texttt{student\_responses.parquet} \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ2 -- Repository density vs. coordination friction}} \\
\midrule
M4 & Repository Activity Density & Semester $\times$ cut & \texttt{code\_churn\_metrics.parquet} \\
M5 & Coordination-Friction Trajectory & Cohort $\times$ cut & \texttt{cohort\_temporal\_friction.csv} \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ3 -- Planning quality, destructive rework, and outcomes (primary)}} \\
\midrule
M6 & $T_1$ Planning-Quality Score & Team-semester & \texttt{team\_level\_signals.csv} \\
M7 & $T_1$ Planning-Omission Rate & Cohort & \texttt{team\_level\_signals.csv} \\
M8 & Rework Severity Ratio & Team-semester & \texttt{team\_level\_signals.csv} \\
M9 & Planning vs. Rework/Outcome Association & Team-semester & \texttt{team\_level\_signals.csv} 

+ \texttt{evaluator\_outcome\_metrics.parquet} \\
\bottomrule
\end{tabularx}
\end{table*}
