...
organized by research question. Every number below traces to a row in
\path{paper_v8/data/results/results_summary.json}, computed by
\path{paper_v8/scripts/results/build_results_summary.py} directly from the
M1--M9 CSVs; no new metric is introduced in this section.
...
% and writes paper_v8/data/results/results_summary.json. Figures are
% produced by paper_v8/scripts/results/generate_figures.py (plotly +
% kaleido, already-pinned dependencies) into paper_v8/figures/. Neither
% script recomputes M1-M9 or issues new LLM calls.
...
$1.21$ at $T_1$ to $1.28$ at $T_3$ ($\Delta{=}+0.07$ on the $[0,4]$ scale),
and in 2026.1 from $1.16$ to $1.32$ ($\Delta{=}+0.17$). Both cohorts show a
small net increase, consistent with growing reliance on generative-AI tools
...
Table~\ref{tab:role_risk} ranks the six surveyed roles by their pooled
perceived risk of AI-driven displacement (M2, response-weighted mean across
all cohort-cuts, $1$--$5$ scale). Contrary to the common assumption that
implementation-heavy or highly proceduralized roles (QA, Backend) are seen
as most automatable, students rank \textbf{Frontend} development as the
role most at risk ($2.81$) and \textbf{Scrum Master} second ($2.43$),
while \textbf{Project Manager} is perceived as least at risk ($2.01$).

\begin{table}[t]
\caption{Perceived AI Role-Disruption Risk (M2), Pooled Across Cohort-Cuts}
\label{tab:role_risk}
\centering
\begin{tabular}{@{}lc@{}}
\toprule
\textbf{Role} & \textbf{Pooled mean risk (1--5)} \\
\midrule
Frontend & 2.81 \\
Scrum Master & 2.43 \\
Backend & 2.32 \\
Product Manager & 2.15 \\
QA & 2.11 \\
Project Manager & 2.01 \\
\bottomrule
\end{tabular}
\end{table}
...
cohorts. Mean total churn escalates from $3{,}615$ lines at $T_1$ to
$964{,}438$ lines at $T_3$ --- a $266.8\times$ increase --- confirming that
repository-visible activity concentrates sharply at the deadline. Over the
same window, the LLM-coded coordination-friction score is constant at
$8/10$ at every checkpoint ($T_1$, $T_2$, and $T_3$ all score $8$), the
central empirical basis for RQ2: friction is not an emergent property of
late-cycle crunch, but a standing condition already near-ceiling at the
very first checkpoint, while repository metadata only becomes a visible
proxy for it much later.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/rq2_activity_density_vs_friction.png}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/rq3_rework_by_planning_group.png}
