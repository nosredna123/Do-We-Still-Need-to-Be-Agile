...
survey or transcript text become the nine metrics (M1--M9) used for RQ1--RQ3.
All metrics derive from artifacts produced by pipeline stages 00--09 and
\path{12_paper_signals_extractor.py}. No new data are collected and no new LLM
calls are issued. For each metric, we identify a reproducible processing path
from the raw artifact to the reported value.
...
\[
\begin{aligned}
\mathrm{AIAuthDensity}(i,s,c)
  &= \underset{a \in A_{\mathrm{AI}}}{\mathrm{median}}
     \left(\frac{C(a,i,s,c)}{C(i,s,c)}\right).
\end{aligned}
\]
...
\textbf{M2 -- AI-Dependency Trajectory.} Six LLM-coded question families
capture benefits, career impact, tool dependency, career expectations,
project challenges, and project feeling. Each uses an ordinal $[0,4]$ scale.
For cohort-cut $(s,c)$, the composite is the unweighted mean across families:
\[
...
\textbf{M3 -- Perceived Role-Disruption Risk.} For each surveyed SE role
(backend, frontend, QA, project manager, product manager, and Scrum Master)
and cohort-cut $(s,c)$, we summarize the raw 5-point Likert item directly from
the survey instrument, with no LLM involved:
\[
...
scores (\path{pipeline_statistics.summarize_numeric_distribution}).
...
$\mathrm{cc\_total}$ and commit count $\mathrm{cc\_commit\_n}$. Let
$\mathcal{S}$ denote the distribution summary. For either metric $m$, we use:
\[
\mathrm{ActivityDensity}(s,c,m)
...
The persisted signals give an omission rate of $55.6\%$ ($5/9$) for 2025.2
and $0\%$ ($0/5$) for 2026.1. The former is the source of the abstract's
``55\% of the first cohort'' claim.
...
M8, and four independent evaluator outcomes at team-semester grain: project
progress, scope, technical complexity, and engagement at $T_3$. Given
$n \leq 14$, we report two descriptive views: (a) Spearman rank
...
Table~\ref{tab:metrics_summary} consolidates M1--M9 by research question. RQ1
metrics triangulate a team-level behavioral signal (M1) against cohort-level
self-report (M2, M3), a grain mismatch stated explicitly rather than papered
over. RQ2 metrics contrast quantitative repository density (M4) against
qualitative coordination friction (M5) on a shared $T_1\text{--}T_3$ axis.
RQ3 metrics form a single evidentiary chain at team-semester grain: an
upfront planning signal (M6), its cohort-level omission rate (M7), a
late-stage destructive-rework signal (M8), and their joint, evaluator-outcome
association (M9). All nine metrics reuse already-persisted, frozen artifacts;
none reruns Phase 1/2 or issues new LLM calls. Concrete values, figures, and
...
\centering
\begin{tabularx}{\textwidth}{@{}p{0.7cm}p{3.6cm}p{2.6cm}X@{}}
\toprule
\textbf{ID} & \textbf{Metric} & \textbf{Grain} & \textbf{Source Artifact (script)} \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ1 -- GenAI adoption and role-perception}} \\
\midrule
M1 & AI-Authorship Density (behavioral) & Team-semester $\times$ cut & \texttt{integration\_friction\_metrics.parquet} (\path{m1_ai_authorship_density.py}) \\
M2 & AI-Dependency Trajectory (perceptual) & Cohort $\times$ cut & \texttt{textual\_cut\_signals.parquet} (\path{m2_ai_dependency_trajectory.py}) \\
M3 & Perceived Role-Disruption Risk & Cohort $\times$ cut & \texttt{student\_responses.parquet} (\path{m3_role_disruption_risk.py}) \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ2 -- Repository density vs. coordination friction}} \\
\midrule
M4 & Repository Activity Density & Semester $\times$ cut & \texttt{code\_churn\_metrics.parquet} (\path{m4_repo_activity_density.py}) \\
M5 & Coordination-Friction Trajectory & Cohort $\times$ cut & \texttt{cohort\_temporal\_friction.csv} (\path{m5_coordination_friction_trajectory.py}) \\
\midrule
\multicolumn{4}{@{}l}{\textit{RQ3 -- Planning quality, destructive rework, and outcomes (primary)}} \\
\midrule
M6 & $T_1$ Planning-Quality Score & Team-semester & \texttt{team\_level\_signals.csv} (\path{m6_t1_planning_quality.py}) \\
M7 & $T_1$ Planning-Omission Rate & Cohort & \texttt{team\_level\_signals.csv} (\path{m7_planning_omission_rate.py}) \\
M8 & Rework Severity Ratio & Team-semester & \texttt{team\_level\_signals.csv} (\path{m8_rework_severity_ratio.py}) \\
M9 & Planning vs. Rework/Outcome Association & Team-semester & \texttt{team\_level\_signals.csv} + \texttt{evaluator\_outcome\_metrics.parquet} (\path{m9_planning_vs_rework_association.py}) \\
\bottomrule
\end{tabularx}
\end{table*}
