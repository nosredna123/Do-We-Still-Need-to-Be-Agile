...
Figure~\ref{fig:rq3} shows the T3 Rework-Churn volume (M8) for a
median-split contrast of planning quality (M9): teams below the median
planning score have a mean $T_3$ rework churn of $21{,}258$ lines, versus
$2{,}732$ lines for teams above the median --- a $7.8\times$ difference.
The single most extreme case, TEAM\_09 (2025.2), incurred $100{,}629$ lines
of $T_3$ rework churn, the source of the abstract's ``100,000+ lines''
claim. The same low-vs-high split shows a modest, narrative-consistent
direction on independent evaluator outcomes: mean $T_3$ project-progress
score is $3.54$ for low-planning teams versus $3.67$ for high-planning teams.

\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{figures/rq3_rework_by_planning_group.png}
\caption{$T_3$ rework-churn volume (log scale) for teams with below- vs.
above-median $T_1$ planning-quality scores.}
\label{fig:rq3}
\end{figure*}

Table~\ref{tab:rq3_correlations} reports the Spearman rank correlations
between the $T_1$ Planning-Quality Score and each outcome across the nine
scored teams. The rework-related outcomes (rework-churn volume, rework
ratio) and the engagement/participation outcome show the expected negative
direction (higher planning quality, less rework), while scope/applicability
and technical complexity are essentially flat. The project-progress
correlation is also negative, the opposite sign from the group-contrast
direction reported above; at $n=9$ this sign instability is expected and is
exactly why we do not treat either statistic as more than descriptive.
Consistent with this project's exploratory-evidence stance (Threats to
Validity), none of these correlations reaches conventional significance at
$n=9$; they are reported as descriptive context alongside the group
contrast above, which remains the primary quantitative basis for the
planning-quality-vs.-rework claim.

\begin{table}[t]
\caption{Spearman Correlations: $T_1$ Planning-Quality Score vs. Outcomes ($n=9$)}
\label{tab:rq3_correlations}
\centering
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Outcome} & \textbf{n} & $\boldsymbol{\rho}$ & \textbf{p} \\
\midrule
Rework-churn volume ($T_3$) & 9 & $-0.31$ & $0.42$ \\
Rework ratio ($T_3$) & 9 & $-0.26$ & $0.51$ \\
Project progress ($T_3$) & 9 & $-0.26$ & $0.51$ \\
Scope/applicability ($T_3$) & 9 & $-0.05$ & $0.90$ \\
Technical complexity ($T_3$) & 9 & $-0.12$ & $0.75$ \\
Engagement/participation ($T_3$) & 9 & $-0.35$ & $0.36$ \\
\bottomrule
\end{tabular}
\end{table}
