...
STAGES = (
    ...
    "cross-evidence", "cross-evidence-report",
)
STAGE_SCRIPTS = {
    ...
    "cross-evidence": "08_cross_evidence_engine.py",
    "cross-evidence-report": "09_cross_evidence_narrative_reporter.py",
}
...
def build_stage_command(
    ...
    if stage == "cross-evidence-report":
        command.extend(
            [
                "--analysis-dir",
                str(PROJECT_ROOT / "data" / "analysis"),
...
def validate_dry_run_requirements(stages: list[str], nlp_backend: str) -> None:
    ...
    if any(stage in stages for stage in ("nlp", "metrics", "stats", "narrative-audit", "cross-evidence", "cross-evidence-report")):
    ...
    if any(stage in stages for stage in ("nlp", "narrative-audit", "cross-evidence-report")) and nlp_backend not in {"openai", "mock"}:
